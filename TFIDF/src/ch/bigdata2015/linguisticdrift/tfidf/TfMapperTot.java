 ...
 *
 */
public class TfMapperTot extends Mapper<LongWritable, 
                                            Text, 
                                            Text, 
                                            IntWritable> { 
    ...
    String fileName;
/**
 * Map function that outputs : 
 * Key : Year 
 * Value : occurence for a word
 */
    public void map(LongWritable inputKey, Text inputValue, Context context)
            throws IOException, InterruptedException {
    	fileName = ((FileSplit) context.getInputSplit()).getPath().getName().substring(0,4);
